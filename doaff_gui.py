...
import sys
import os
import distance_estimation  #backend file
...
def run_code():
    ...
    def worker():
        try:
            distance_estimation.start_with_coordinates(clat, clon)
...
def section_label(parent, text):
    ...
    tk.Label(row, text="▸ " + text, font=FONT_LABEL,
             bg=PANEL, fg=AMBER).pack(side=tk.LEFT)


# ── root window ───────────────────────────────────────────────────────────────
root = tk.Tk()
root.title("IndoWings Artillery Blast Distance Estimator")
...
_logo_path = "/home/nisha-/yolov5_doaff/indowings_logo.png"
try:
...
tk.Label(title_row,
         text="Artillery Blast Distance Estimator",
         font=("Courier New", 15, "bold"), bg=PANEL, fg="#b8860b").pack(side=tk.LEFT)
...
def coord_row(parent, label_text, row_n):
    ...
    tk.Label(row, text=label_text,
             ...
             bg=PANEL, fg=MUTED).pack(side=tk.LEFT, padx=(14, 6), pady=10)
...
tk.Label(status_row, text="STATUS", font=("Courier New", 8, "bold"),
...
for col in [RED_ERR, AMBER, GREEN]:
    tk.Label(chrome, text=" ● ", font=("Courier New", 8),
             bg="#181c22", fg=col).pack(side=tk.LEFT, padx=1)
tk.Label(chrome, text="STREAM ACTIVE", font=("Courier New", 8),
         bg="#181c22", fg=MUTED).pack(side=tk.LEFT, padx=8)

inner = tk.Frame(terminal_outer, bg=BG)
inner.pack(fill=tk.BOTH, expand=True)

scrollbar = tk.Scrollbar(inner, bg=PANEL, troughcolor=BG,
...
output_box.bind("<MouseWheel>",
    lambda e: output_box.yview_scroll(int(-1*(e.delta/120)), "units"))

# ── terminal background image ─────────────────────────────────────────────────
try:
    _tm_raw = Image.open("/home/nisha-/yolov5_doaff/gui_logo.jpg").convert("RGB")
...
tk.Label(foot,
         text="RESTRICTED  //  AUTHORISED PERSONNEL ONLY",
         font=("Courier New", 7, "bold"),
         bg=PANEL, fg=AMBER_DIM).pack(pady=3)
